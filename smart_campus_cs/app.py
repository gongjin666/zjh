...
def build_graph():
    ...
    for name in honors: G.add_node(name, type="荣誉年份")  # 年份作为节点
    for name in leaders: 
        if isinstance(leaders[name], list):
            for sub in leaders[name]: G.add_node(sub, type="领导")
        else:
    ...
    for year in honors: G.add_edge("吉利学院", year, relation="荣誉年份")
    for leader_name in [leaders["董事长"], leaders["校长"], leaders["党委书记、督导专员、副校长"], leaders["党委副书记、副校长"]]:
        G.add_edge("吉利学院", leader_name, relation="领导")
    for campus in campuses: G.add_edge("吉利学院", campus, relation="校区")
    for culture in campus_culture: G.add_edge("吉利学院", culture, relation="文化")
    for cat in clubs: G.add_edge("吉利学院", cat, relation="社团类别")
...
if prompt := st.chat_input("请输入您的问题..."):
    ...
    all_entities = {**courses, **facilities, **policies, **history, **famous_alumni, **honors, "吉利学院": None, **leaders, **campuses, **campus_culture}
